...
import { uploadToImageServer } from "../../../../../utils/image-upload"; // Import the utility
...
export const EditProductMediaForm = ({ product }: ProductMediaViewProps) => {
  ...
  const handleSubmit = form.handleSubmit(async ({ media }) => {
    // Filter out any entries that might still have a file attached (shouldn't happen with new upload flow, but good safeguard)
    const validMedia = media.filter(m => m.url);

    const thumbnail = validMedia.find((m) => m.isThumbnail)?.url

    await mutateAsync(
      {
        // Map only the necessary properties (URL and existing ID)
        images: validMedia.map((file) => ({ url: file.url, id: file.id })),
        thumbnail: thumbnail || null, // Use null if no thumbnail is set
      },
      {
        onSuccess: () => {
