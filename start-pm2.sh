...
if grep -q "NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY=" .env && ! grep -q "NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY=$" .env; then
  ...
  npm run build && pm2 start ecosystem.config.js --only damned-designs-storefront
else
  ...
  echo "cd storefront && npm run build && cd .. && pm2 start ecosystem.config.js --only damned-designs-storefront"
fi
cd ..
