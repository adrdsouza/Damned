...
export const Sorting = (sorttype: number, data:Product[]): Product[] => {    
  switch(sorttype.toString()) {
    ...
    case '2':                      
      return data.sort((a,b) => new Date(b.node.image && b.node.image.date).getUTCDate() - new Date(a.node.image && a.node.image.date).getUTCDate());
    case '3':        
    ...
    case '0':     
    default:
      return data.sort((a,b) => a.node.name.toLowerCase().localeCompare(b.node.name.toLowerCase()))
  }
}
